<template>
  <div id="flash-food">
    <div class="flash-item-wrapper">
      <ul class="item-wrapper" ref="ulWrapper">
        <li class="item-in-covers"
            v-for="(product, index) in flash_sale_product_list"
            :key="index"
            ref="productItem"
            @click="goToGoodsDetail(product)">
          <img v-lazy="product.small_image" class="item-image">
          <span class="title">{{ product.name }}</span>
          <div class="price">
            <p class="now-price">{{ product.price | moneyFormat }}</p>
            <p class="origin-price">{{ product.origin_price | moneyFormat }}</p>
            <div class="buy-car" @click.stop="addToCart(product,index)">
...
#flash-food {
  .flash-item-wrapper {
    ...
    .move-dot {
      ...
      border-radius: 50%;
      img {
        animation: 0.88s my-move ease-in-out;
        width: 3rem;
        height: 3rem;
        border-radius: 50%;
      }
