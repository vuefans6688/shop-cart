<template>
  <div class="wrapper">
    <!-- 可滑动的标题 -->
    <div class="sub-title-wrapper" ref="sub-title-wrapper">
      <ul ref="ul-content">
        <li class="title"
    ...
    <div class="show-menu"
      @click="menuClick"
      v-if="isShowDropMenu">
      <span class="down-menu" v-if="menuDown">
        <svg-icon iconClass="down" style="width: 1.5rem;height: 1.5rem;" />
      </span>
      <span class="up-menu" v-else>
        <svg-icon iconClass="up" style="width: 1.5rem;height: 1.5rem;" />
      </span>
    </div>
    <!-- 下拉菜单内容 -->
    ...
    <!-- 商品内容列表 -->
    <section class="r-list" ref="r_list">
      <div ref="b">
        <div v-for="(it, index) in categoriesDetailData" :key="index"
          ref="good">
          <p class="product-category-title">{{ it.name }}</p>
          <ul>
            <li v-for="(item, index) in it.products"
              ...
              <div class="list-item flex">
                ...
                <div>
                  ...
                  <p class="origin-price">{{ item.origin_price | moneyFormat }}</p>
                  <div class="icon-cart-wrapper" @click.stop="addToCart(item)">
        ...
        <div class="bottom-tip">{{ $t('category.bottomTip') }}</div>
      </div>
    </section>
  </div>
</template>
