<template>
  <div id="login">
    <div class="login-box" @click="changeImage(3)">
      <div class="auth-form">
        <div class="pan-fish">
          <img :src="imageURL" alt="">
        </div>
        <!-- 关闭按钮 -->
        <van-icon name="close" class="close-button" @click="close" />
        <van-tabs v-model="active" animated>
          <van-tab :title="$t('login.title')">
            ...
            <div class="switch-login" @click="switchLogin">
              {{ this.isShowSMSLogin ? $t('login.phoneVerify') : $t('login.smsMessage')}}
            </div>
          </van-tab>
          <!-- 注册 -->
        ...
        <p class="agreement">{{ $t('login.tipTile') }}<br>{{ $t('login.tipContent') }}
          <a @click.stop="agreement(0)" class="agreement-box">{{ $t('login.tip') }}</a>、
...
// 引入API调用接口
import { getPhoneCaptcha, phoneCaptchaLogin } from '../../serve/api/index'
// 引入vuex
...
export default {
  ...
  methods: {
    ...
    async login () {
      if (this.isShowSMSLogin) {
        ...
        if (this.login_userName.length < 1) {
          ...
          return;
        } else if (!this.phoneRegExp(this.login_userName)) {
          Toast({
    ...
    async register () {
      ...
      } else if (!this.phoneRegExp(this.register_userName)) {
        Toast({
          message: this.$t('login.phoneNumberNotCorrect'),
          duration: 800
        })
